...
// 3. Armar array con la estructura deseada
...
$geoFinal = [
    ...
    'lang'        => ($geo['countryCode'] ?? 'BR') == 'BR' ? 'PT' : 'EN',

    'langFunny'   => ($geo['countryCode'] ?? 'BR') == 'BR' ? 'PORTUGUÉS' : 'INGLÉS'
...
// Traer servicios destacados con sus coordenadas y calcular distancia
...
JOIN (
    ...
    -- Subconsulta: salida más cercana
    ...
    ) ss2 ON ss1.idServicio = ss2.idServicio AND ss1.fecha = ss2.proxima_fecha

) ss ON s.idServicio = ss.idServicio

JOIN servicio_tarifas_ubicacion u ON u.idServicioTarifasUbicacion = ss.idServicioSalidas
...
ORDER BY s.destacado DESC, distancia_km ASC
...
// Preparar la consulta para usar parámetros
...
$stmt->bind_param("ddd", $latUsuario, $lonUsuario, $latUsuario);
...
$servicios_geo = $result->fetch_all(MYSQLI_ASSOC);

$stmt->close();



// Función para calcular distancia en PHP por si necesitas reordenar
...
// Evitar duplicados por servicio

$servicios_unicos = [];

foreach ($servicios_geo as $s) {

    $id = $s['idServicio'];

    if (!isset($servicios_unicos[$id])) {

        $servicios_unicos[$id] = $s;

    }

}

$servicios_geo = array_values($servicios_unicos);



// Ordenar servicios por distancia usando PHP (opcional)
...
$servicios = array_slice($servicios_geo, 0, 6); // Primeros 6 servicios del array ordenado
...
$servicios = array_slice($servicios_geo, 6); // El resto de servicios
...
<!-- BUCLE VER MAS-->
  ...
  <!--FINALIZA DESTOPK SECCION Actividades destacadas-->
