...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $email=$_POST['email'];
$codigoGet=$_POST["codigo"];
$usuario=getUsuarioEmail($email);
if (count($usuario)==1 && $codigoGet==$usuario[0]['codigoRecuperacion']) {
 // print_r($_POST);
  if ($_POST["contrasena"]==$_POST["confirma_contrasena"]) {
    $clave=$_POST["contrasena"];
    $clave=md5($clave);
    $idUsuario=$usuario[0]["idUsuario"];
     $resuset=setPassword($idUsuario, $clave);
     setCodigoRecuperacion($idUsuario, 0);

     //echo "string". $resuset;
     if ($resuset>0) {
       alertar("contraseña actualizada con exito, ya puede hacer login nuevamente", "success");
       redireccionarLento("index");
     }
  }
  else{
    alertar("Las contraseñas no coinciden", "warning");
  }
      ...
      <div class="col-lg-12">
        ...
        <div class="badge badge-primary badge-ciudad"></div>
         <!--FIN BUCLE DE LOS RESULTADOS AQUI-->
          ...
          <!--TITULO-->
        <h1 class="text-white texto-shadow py-2 bold" style=" text-shadow: -1px 0px 6px #000000; text-align:center;"><?=$lang["altere_senha"];?></h1>
         <!--TITULO-->
...
<head>
 ...
 <h3 class="py-2 bold" style="text-align:center;"><?=$lang["ultimo_paso"];?></h3>
...
<div class="login-box" style="margin-left: 20%;margin-right: 20%;">
  ...
  <div class="card">
    <div class="card-body login-card-body">
      <p class="login-box-msg"><?=$lang["para_iniciar_el_proceso_redefinicion"];?></p>

      <form method="post">
                  <input type="hidden" name="email" value="<?=$email;?>">
          <input type="hidden" name="codigo" value="<?=$codigoGet;?>">
        <div class="input-group mb-3">
          <input type="password" class="form-control" name="contrasena" placeholder='<?=$lang["nueva_senha"];?>'>
        ...
        <div class="input-group mb-3">
          <input type="password" class="form-control" name="confirma_contrasena" placeholder="<?=$lang["repita_senha_nueva"];?>">
          <div class="input-group-append">
            <div class="input-group-text">
              <span class="fas fa-lock"></span>
            </div>
        ...
        <div class="row">
          <div class="col-12">
            <button type="submit" class="btn btn-primary btn-block"><?=$lang["confirmar_senha_nueva"];?></button>
          </div>
          <!-- /.col -->
        </div>
      </form>
      ...
      <p class="mt-3 mb-1">
        <a href="login.html"><?=$lang["registrarse"];?></a>
      </p>
    </div>
    <!-- /.login-card-body -->
  </div>
