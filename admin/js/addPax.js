...
function addPax(){
                                    			...
                                    			var txtDireccion=$("#txtDireccion2").val();//        var txtDireccion=$("#txtDireccion2").val();
...
if (true) { //txtDireccion.length>0
	if (nombre_tarifa.length>0) {

		if (true) {//valor_tarifa>0 && tipo_tarifa!=4
			posicion +=1;

	       $('#tipos_pax_table').append('<tr id="tTarifas'+posicion+'"><td>'+txtDireccion+'</td>'+
	       	 							 '<td>'+nombre_tarifa+'</td>'+
        								 ...
        								 '<td>'+valor_tarifa+'</td>'+
        								 '<td>'+valor_tarifa+'</td>'+
        								 '<td>'+selCancelaciones_text.substr(0, 50)+'...</td>'+
                                         '<td>'+comisionaFunny+'</td> '+
                                         '<td onclick="eliminarFila('+posicion+')"><button type="button" class="btn btn-sm btn-danger">-</button>                                         </td>'+'<input type="hidden" name="txtDireccion[]" value="'+txtDireccion+'"></input>'+
...
Swal.fire({
  title: 'Você realmente deseja remover a taxa?',
