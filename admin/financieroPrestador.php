  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response.success && response.pasajeros && response.pasajeros.length > 0) {
        var html = '<table class="table table-sm table-bordered"><thead><tr><th>Pasajero</th><th>Tarifa</th><th>Valor Referente a Comisión</th><th>Comision Reservate %</th><th>A Pagar Prestador</th></tr></thead><tbody>';
        response.pasajeros.forEach(function(p) {
          var porcTotal = (p.comisionTotalPorc !== undefined && p.comisionTotalPorc !== null) ? Number(p.comisionTotalPorc).toFixed(2) + '%' : 'N/A';
          var valorSinImp = p.valorSinImpuestos || 'N/A';
          html += '<tr><td>' + p.nombre + '</td><td>' + p.tarifa + '</td><td>' + valorSinImp + '</td><td>' + porcTotal + '</td><td class="text-success font-weight-bold">' + (p.aPagar || 'N/A') + '</td></tr>';
        });
        html += '</tbody></table>';
