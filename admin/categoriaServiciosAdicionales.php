                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <form method="post">
                                                    ...
                                                    <button type="submit" class="btn btn-danger" name="eliminarAdicional" value="<?= $habilitados[$i]['idServiciosAdicionalesCategoria']; ?>">Quitar</button>
                                                </form>
                                            </td>
                                        </tr>
                                    <?php } ?>
                    ...
                    <div class="row">
                        <div class="table-responsive">
                            <table class="table" id="example2">
                                ...
                                <tbody>
                                    <?php for ($i = 0; $i < count($noHabilitados); $i++) { ?>
                                        <tr>
                                            <td><?= $noHabilitados[$i]["nombre"]; ?></td>
                                            <td>
                                                <form method="post">
                                                    <input type="hidden" name="idCategoria_servicio" value="<?= $idCategoria_servicio; ?>">
                                                    <button type="submit" name="addAdicional" value="<?= $noHabilitados[$i]['idServiciosAdicionales']; ?>" class="btn btn-success">Agregar</button>
                                                </form>
                                            </td>
                                        </tr>
                                    <?php } ?>
...
<script type="text/javascript">
    ...
    function borrar(idABorrar) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.post("./ctrl/ctrl_comentarios_categoria.php", parametros, function (data, status) {
                    console.log(data);
                    if (data > 0) {
