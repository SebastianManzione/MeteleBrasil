...
if ($_SERVER["REQUEST_METHOD"]=="POST" && isset($_POST["nota"])) {
  ...
  if (strlen($_POST["nota"])>3) {
    ...
    $notaReserva= altaNotaReserva($_SESSION["login"]['idUsuario'], $_POST["nota"],$_POST["idReserva"]);
...
<tr><td colspan="2">'.$cantidad.' '.$nombre_tarifa.' '.$fromEdad.' a '.$toEdad.' Anos</td></tr>';
 ...
 for ($k=0; $k < count($pasajeros); $k++) { 
 	  ...
 	  <tr>
                                            ...
                                            <td>'. $pasajeros[$k]["nombrePasajero"].'</td>

                                          <td>'. $pasajeros[$k]["apellidoPasajero"].'</td>
