...
class AdminMenu {
    ...
    public function getMenuTreeForRole($roleId, $session = []) {
        ...
        // Si no hay permisos definidos para los roles detectados, no mostrar menú (evita fallback liberal)
        if (!$hasPermissionsForRoles) {
            return [];
        }
        
        // Query: Trae items con permiso + sus ancestros (padres necesarios)
    ...
    /**
     * Obtiene todo el menú (sin filtrar por roles) para usar en administración
     */
    public function getMenuTreeAll() {
